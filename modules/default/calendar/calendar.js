		...
		for (var e in events) {
			...
			if (event.fullDayEvent) {
				if (event.today) {
					...
					if (this.config.timeFormat === "absolute") {
						if ((this.config.urgency > 1) && (event.startDate - now < (this.config.urgency * one_day))) {
							...
							timeWrapper.innerHTML = moment(event.startDate, "x").format("dddd");   /// THIS IS THE ONE
						}
					} else {
						timeWrapper.innerHTML =  moment(event.startDate, "x").fromNow();
					}
				...
				if (event.startDate >= new Date()) {
					if (event.startDate - now < 6 * one_day) {
						...
						if (event.startDate - now < 6 * one_hour) {
							...
							timeWrapper.innerHTML = moment(event.startDate, "x").calendar(); //within a week
						}
					} else {
						/* Check to see if the user displays absolute or relative dates with their events
						 * Also check to see if an event is happening within an 'urgency' time frameElement
						...
						if (this.config.timeFormat === "absolute") {
							if ((this.config.urgency > 1) && (event.startDate - now < (this.config.urgency * one_day))) {
								...
								timeWrapper.innerHTML = moment(event.startDate, "x").format("MMM Do");  //outside of a week
							}
						} else {
							timeWrapper.innerHTML = moment(event.startDate, "x").fromNow();
						}
